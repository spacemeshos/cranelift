...
use cranelift_codegen::binemit::{Addend, CodeOffset, NullTrapSink, Reloc, RelocSink, NullStackmapSink};
...
/// `ObjectBuilder` means that `ObjectCompiledFunction`s will contains trap sites.
pub enum ObjectTrapCollection {
    /// `ObjectCompiledFunction::traps` will be empty
    Disabled,
    /// `ObjectCompiledFunction::traps` will contain trap sites
    Enabled,
}
...
impl ObjectBuilder {
    ...
    ///
    /// `collect_traps` setting determines whether trap information is collected in each
